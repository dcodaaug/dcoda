...
class URRobot(object):
    ...
    def _wait_for_move(self, target, threshold=None, timeout=0.6, joints=False):
        """
        ...
        while True:
            ...
            if not self.secmon.is_program_running():
                ...
                if count > timeout * 10:
                    ...
                    # print("Goal not reached for {} seconds. dist is {}, threshold is {}, target is {}, current pose is {}".format(timeout, dist, threshold, target, URRobot.getl(self)))
                    return
            else:
                count = 0
    ...
    def movex(self, command, tpose, acc=0.01, vel=0.01, wait=True, relative=False, threshold=None):
        ...
        if relative:
            l = self.getl()
            tpose = [v + l[i] for i, v in enumerate(tpose.array)]
        prog = self._format_move(command, tpose.array, acc, vel, prefix="p")
        self.send_program(prog)
        if wait:
            self._wait_for_move(tpose.array[:6], threshold=threshold)
            return self.getl()
