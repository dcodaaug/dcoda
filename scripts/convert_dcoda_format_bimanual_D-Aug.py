...
def save_images_to_dcoda_folder(dcoda_traj_images_folder, ep_folder_path):
    ...
    for i, rgb_file in enumerate(wrist_left_rgb_files):
        ...
        image_filename = f"wrist_left_{i:05d}.png"
    ...
    for i, rgb_file in enumerate(wrist_right_rgb_files):
        ...
        image_filename = f"wrist_right_{i:05d}.png"
...
def save_absolute_poses_to_dcoda_folder(dcoda_traj_images_folder, ep_folder_path, low_dim_obs):
    ...
    for wrist_left_rgb_file, wrist_right_rgb_file in zip(wrist_left_rgb_files, wrist_right_rgb_files):
        wrist_left_image_filename = f"wrist_left_{i:05d}.png"
        raw_labels[wrist_left_image_filename] = low_dim_obs[i].perception_data['wrist_left_pose'].tolist()
        wrist_right_image_filename = f"wrist_right_{i:05d}.png"
        ...
        camera_extrinsics[wrist_left_image_filename] = low_dim_obs[i].misc['wrist_left_camera_extrinsics'].tolist()
        camera_extrinsics[wrist_right_image_filename] = low_dim_obs[i].misc['wrist_right_camera_extrinsics'].tolist()
    ...
    camera_extrinsics_filepath = os.path.join('/'.join(dcoda_traj_images_folder.split('/')[:-1]), 'camera_extrinsics.json')
    with open(camera_extrinsics_filepath, 'w') as json_file:
        json.dump(camera_extrinsics, json_file, indent=4)  # indent=4 makes the file more readable
    print(f'Finished saving {camera_extrinsics_filepath}')
...
def save_relative_poses_to_dcoda_folder(dcoda_traj_images_folder, ep_folder_path, low_dim_obs, intervals):
    ...
    for i, frame_idx in enumerate(range(start_frame, end_frame)):
        ...
        w_l_frame_name = f"wrist_left_{frame_idx:05}.png"
        w_l_frame_namelk = f"wrist_left_{goal_frame:05}.png"
        ...
        w_r_frame_name = f"wrist_right_{frame_idx:05}.png"
        w_r_frame_namelk = f"wrist_right_{goal_frame:05}.png"
