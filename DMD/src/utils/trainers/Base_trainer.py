...
class Base_trainer:
    def __init__(self,compute_only=True):
        ...
        self.print_iteration = 200

        if not self.compute_only:
            log_dir = os.path.join(globals.instance_data_path,f'logs/{time.strftime("%Y-%m-%d_%H_%M_%S", time.localtime())}')
    ...
    def train(self):
        for epochs_completed in range(self.epoch, self.max_epoch):
            ...
            if not self.compute_only:
                ...
                expected_checkpoint = os.path.join(globals.ckpt_path, f"{self.epoch:04d}-{0:08d}.pth")
                #f'../instance-data/checkpoints/{self.epoch:04d}-{0:08d}.pth'
    ...
    def save_checkpoint(self,checkpoint_filename,dataloader_iter=None):
        checkpoint_root = globals.ckpt_path #'../instance-data/checkpoints'
        ...
        if self.checkpoint_retention_interval > 0: # ignore any  matching retention criterea
            ...
            for x in checkpoints:
                xp1,xp2 = x.split(".")[0].split("-")
                xp1_int = int(xp1)
                if xp1_int % self.checkpoint_retention_interval > 0:
                    checkpoints_new.append(x)
                if int(xp2[:-4]) > 0:
                    checkpoints_new.append(x)
