...
class NCSNpp_dual_bimanual(nn.Module):
  """NCSN++ model"""
  ...
  def __init__(self, config):
    ...
    CrossAttnBlock = functools.partial(CrossAttnBlockpp.CrossAttnBlockppTwoCams,
                                  init_scale=init_scale,
                                  skip_rescale=skip_rescale,
                                  left_right_pose_cond=config.left_right_pose_cond)
    ...
    print('Include pose conditioning between left and right wrists:', config.left_right_pose_cond)
    self.include_gripper = config.gripper_state
    print('Include gripper state:', self.include_gripper)
    if self.include_gripper:
      # left-armed gripper open and right-armed gripper open
      self.gripper_input_dimension = 2
      self.gripper_proj_dimension = 64
      self.gripper_proj_layer = layerspp.LinearProjection(self.gripper_input_dimension, self.gripper_proj_dimension)
    ...
    if self.include_gripper:
      # ============================================== time embedding pre processing
      if conditional:
        self.temb_modules.append(nn.Linear(embed_dim, (nf * 4) - self.gripper_proj_dimension))
        self.temb_modules[-1].weight.data = default_initializer()(self.temb_modules[-1].weight.shape)
        nn.init.zeros_(self.temb_modules[-1].bias)
        self.temb_modules.append(nn.Linear((nf * 4) - self.gripper_proj_dimension, (nf * 4) - self.gripper_proj_dimension))
        self.temb_modules[-1].weight.data = default_initializer()(self.temb_modules[-1].weight.shape)
        nn.init.zeros_(self.temb_modules[-1].bias)
    else:
      # ============================================== time embedding pre processing
  ...
  def forward(self, w_l_x_a, w_l_x_b, w_l_time_cond_a, w_l_time_cond_b, w_l_rays_ref, w_l_rays_a, w_l_rays_b, w_r_x_a, w_r_x_b, w_r_time_cond_a, w_r_time_cond_b, w_r_rays_ref, w_r_rays_a, w_r_rays_b, gripper_state=None):
    # both frames need to be processed symmetrically, so we stack them and treat them as a superbatch
    assert w_l_x_a.shape == w_l_x_b.shape == w_r_x_a.shape == w_r_x_b.shape
    b,c,h,w = w_l_x_a.shape
    x = torch.cat([w_l_x_a[:,None,:,:,:],w_l_x_b[:,None,:,:,:],w_r_x_a[:,None,:,:,:],w_r_x_b[:,None,:,:,:]],1).reshape(b*4,c,h,w)
    time_cond = torch.cat([w_l_time_cond_a[:,None],w_l_time_cond_b[:,None],w_r_time_cond_a[:,None],w_r_time_cond_b[:,None]],1).reshape(b*4)
    ...
    if self.include_gripper:
      gripper_state = self.gripper_proj_layer(gripper_state)
      gripper_state = gripper_state.repeat(4, 1)
      temb = torch.cat([temb, gripper_state], 1)
    ...
    # ========== create conditioning pyramid ==========
    cond_0 = self.conditioning_conv_0(self.act(torch.cat([w_l_rays_ref, w_r_rays_ref, w_l_rays_a, w_r_rays_a, w_l_rays_b, w_r_rays_b],0))) # torch.Size([batch*3*3, 128, 32, 32])
    cond_1 = self.conditioning_conv_1(self.act(cond_0))                                 # torch.Size([batch*3*3, 128, 16, 16])
    cond_2 = self.conditioning_conv_2(self.act(cond_1))                                 # torch.Size([batch*3*3, 128, 8, 8])
    cond_3 = self.conditioning_conv_3(self.act(cond_2))                                 # torch.Size([batch*3*3, 128, 4, 4])

    # extract features of rays_ref (index 0), rays_a (index 1), rays_b (index 2)
    conditioning_stack = {
      32: [cond_0[0:b*2,...],cond_0[b*2:b*2*2,...],cond_0[b*2*2:b*2*3,...]],
      16: [cond_1[0:b*2,...],cond_1[b*2:b*2*2,...],cond_1[b*2*2:b*2*3,...]],
      8: [cond_2[0:b*2,...],cond_2[b*2:b*2*2,...],cond_2[b*2*2:b*2*3,...]],
      4: [cond_3[0:b*2,...],cond_3[b*2:b*2*2,...],cond_3[b*2*2:b*2*3,...]],
    }
    # =============================================
    ...
    # unstack super batch
    b,c,hi,wi = w_l_x_a.shape
    h = h.reshape(b,4,c,hi,wi)
    h_a = h[:,0,:,:,:]
    h_b = h[:,1,:,:,:]
    h_c = h[:,2,:,:,:]
    h_d = h[:,3,:,:,:]
