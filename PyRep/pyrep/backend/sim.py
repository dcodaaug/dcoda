...
def simScaleObject(handle, x, y ,z):
    lib.simScaleObject(handle, x, y, z, 0)
    return

def simGetObjectSizeFactor(handle):
    return lib.simGetObjectSizeFactor(handle)
...
def simGetObjectPose(objectHandle, relativeToObjectHandle):
    ...
    _check_return(ret)
    quaternion = list(quaternion) # qw qx qy qz
    qw = quaternion[0]
    qx = quaternion[1]
    qy = quaternion[2]
    qz = quaternion[3]
    ...
    extrinsic_matrix[:3, :3] = rotation_matrix                          # Set the rotation part
    extrinsic_matrix[:3, 3] = [position[0], position[1], position[2]]   # Set the translation part
