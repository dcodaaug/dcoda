...
class BimanualSweepToDustpanEasy(BimanualTask):
    ...
    def init_episode(self, index: int) -> List[str]:
        # bimanual_sweep_to_dustpan_easy
        if sim.simGetObjectSizeFactor(Shape('dirt_boundary').get_handle()) >= 1.0:
            sim.simScaleObject(Shape('dirt_boundary').get_handle(), 0.8, 0.8, 0.8)
