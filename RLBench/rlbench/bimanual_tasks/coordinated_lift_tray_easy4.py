...
class CoordinatedLiftTrayEasy4(BimanualTask):
    ...
    def init_episode(self, index) -> List[str]:
        ...
        if sim.simGetObjectSizeFactor(Shape('workspace').get_handle()) >= 1.0:
            sim.simScaleObject(Shape('workspace').get_handle(), 0.5, 0.68, 0.7)

        b = SpawnBoundary([self.boundaries])
        b.clear()
        b.sample(Dummy('coordinated_lift_tray_easy4'), min_rotation=(0.0, 0.0, 0.0), max_rotation=(0.0, 0.0, 0.0), min_distance=0.0)
    ...
    def boundary_root(self) -> Object:
        return Dummy('coordinated_lift_tray_easy4')
