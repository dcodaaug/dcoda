...
class CoordinatedPushBoxEasy4(BimanualTask):
    ...
    def init_episode(self, index) -> List[str]:
        ...
        if sim.simGetObjectSizeFactor(Shape('workspace').get_handle()) >= 1.0:
            sim.simScaleObject(Shape('workspace').get_handle(), 0.95, 0.95, 0.95)

        angle = np.deg2rad(10)
        b = SpawnBoundary([self.boundaries])
        b.clear()
        b.sample(Dummy('coordinated_push_box_easy4'), min_rotation=(0.0, 0.0, -angle), max_rotation=(0.0, 0.0, angle), min_distance=0.0)
