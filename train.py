...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.framework.wandb_logging:
        ...
        wandb_obs_config = obs_config.__dict__
        final_wandb_config = {**wandb_cfg, **wandb_obs_config}
        final_wandb_config['weights_folder'] = weights_folder
        task_name = os.getcwd().split('/')[-2]
        method_name = os.getcwd().split('/')[-1]
        run_name = task_name + "_" + method_name
        wandb_run = wandb.init(
            project="biaug",
            config=final_wandb_config,
            name=run_name,
        )
