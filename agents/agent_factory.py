...
supported_agents = {
    ...
    "bimanual": ("BIMANUAL_PERACT", "ACT_BC_LANG", "DIFFUSION"),
    "unimanual": (),
}


def create_agent(cfg: DictConfig, eval_cfg=None) -> Agent:
    ...
    if agent_type == "leader_follower":
        ...
        return agent_fn(cfg, eval_cfg=eval_cfg)
    else:
...
def agent_fn_by_name(method_name: str) -> Agent:
    ...
    elif method_name.startswith("DIFFUSION"):
        from agents import diffusion

        return diffusion.launch_utils.create_agent
