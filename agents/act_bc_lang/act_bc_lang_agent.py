...
class ActBCLangAgent(Agent):
    def __init__(
        ...
        if train_cfg is not None:
            try:
                self.temporal_agg = train_cfg.method.temporal_agg
            except:
                self.temporal_agg = True
            print('Temporal aggregation: ', self.temporal_agg)
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        if self._timestep % query_freq == 0:
            with torch.no_grad():
                ...
                # for debugging stacked_rgb...
                # import matplotlib.pyplot as plt
                # # Remove extra dimensions: [1, 1, 3, 128, 128] → [3, 128, 128]
                # img_tensor = stacked_rgb[0][1]; img_np = img_tensor.permute(1, 2, 0).cpu().numpy(); img_np = (img_np - img_np.min()) / (img_np.max() - img_np.min())                
        ...
        if self.temporal_agg:
            ...
            raw_action = self._all_actions[:, t % query_freq]
