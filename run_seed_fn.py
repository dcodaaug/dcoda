...
def run_seed(
    ...
    if cfg.method.name == "ARM":
    ...
    elif cfg.method.name.startswith("DIFFUSION"):
        from agents import diffusion

        assert cfg.ddp.num_devices == 1, "DIFFUSION only supports single GPU training"
        replay_buffer = diffusion.launch_utils.create_replay(
            cfg.replay.batch_size,
            cfg.replay.timesteps,
            cfg.replay.prioritisation,
            cfg.replay.task_uniform,
        ...
        diffusion.launch_utils.fill_multi_task_replay(
            cfg,
            obs_config,
            rank,
            replay_buffer,
